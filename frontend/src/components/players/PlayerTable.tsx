...
export function PlayerTable({ players, sort, order, onSort, compact }: Props) {
  ...
  return <div className="table-scroll"><table className={`data-table ${compact ? 'table-compact' : ''}`}>
    ...
    <tbody>{players.map((player, index) => <tr key={player.id}><td className="rank-col muted">{index + 1}</td><td className="player-col"><Link to={`/players/${player.id}`}><strong>{player.fullName}</strong><small>{player.age ?? '—'} years</small></Link></td><td>{player.team ? <Link className="team-code" to={`/teams/${player.team.id}`}>{player.team.abbreviation}</Link> : <span className="team-code">—</span>}</td><td>{player.primaryPosition ?? '—'}</td><td className="numeric">{formatNumber(player.gamesPlayed)}</td><td className="numeric">{formatNumber(player.goals)}</td><td className="numeric">{formatNumber(player.assists)}</td><td className="numeric"><strong>{formatNumber(player.points)}</strong></td><td className="numeric priority-low">{formatNumber(player.gameScore, 1)}</td><td className="numeric">{formatNumber(player.gameScorePer60, 2)}</td><td className="numeric value-cell"><strong>{formatNumber(player.hockeyValue, 1)}</strong></td><td className="numeric priority-medium">{formatCurrency(player.capHitCents, true)}</td><td className="priority-medium"><StatusBadge status={player.rosterStatus} /></td></tr>)}</tbody>
  </table></div>
}
